...
**Slice 3.4 (text extraction) is IMPLEMENTED, TESTED (including a
pre-merge correctness-review fix), committed, pushed, and open as PR
...
above. **Slice 3.4 (text extraction), including a pre-merge
correctness-review fix, is implemented, tested, committed, pushed, and
open as PR #21 — CI 4/4 green, not yet merged** — see `HANDOFF.md` for
exact status. The next work, in order:
