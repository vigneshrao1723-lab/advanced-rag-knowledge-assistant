...
**merged to `main`** — PR #16, squash commit `e1c4858`. **`main`/
`origin/main` are currently at `e1c4858`.** GitHub Issue #3 (Knowledge
Ingestion) has started: **Slice 3.1 (document data model + migration) is
IMPLEMENTED, TESTED, COMMITTED, and PUSHED, with an open PR awaiting
review** — not yet merged. See "Component status" and "Immediate
priorities" below, and `HANDOFF.md` for exact detail.
...
`main` as squash commit `e1c4858`**. **`main`/`origin/main` are currently
at `e1c4858`.** GitHub Issue #3 Slice 3.1 (document data model +
migration `0004`) is now implemented on branch
`issue-3-slice-3-1-document-schema`, cut from `e1c4858`, with a PR open
and awaiting review — not yet merged; see `HANDOFF.md` for its exact
commit hash and PR number. The old `issue-redis-rate-limiting`,
...
(Knowledge Ingestion) has started: Slice 3.1 adds the `documents`/
`document_chunks` database schema (migration `0004`) — schema only, no
pipeline code — no upload API, storage, extraction, chunking, embedding,
...
| Document upload & ingestion pipeline | PARTIALLY IMPLEMENTED (schema only) | GitHub Issue #3, Slice 3.1: `documents`/`document_chunks` tables exist (migration `0004`, `backend/app/models/document.py`/`document_chunk.py`) — a native `DocumentStatus` enum matching the documented lifecycle (`UPLOADED → PROCESSING → PARSED → CLEANED → CHUNKED → EMBEDDED → INDEXED → READY/FAILED`), workspace-scoped ownership and cascade delete, `UNIQUE(workspace_id, checksum_sha256)`/`UNIQUE(storage_key)`/`UNIQUE(document_id, chunk_index)` constraints. Deliberately no embedding column yet (see `docs/DATA_MODEL.md`). **No upload API, storage abstraction, text extraction, chunking, background processing, or embedding code exists** — those are later slices. Open PR, not yet merged — see `HANDOFF.md`. |
...
— PR #16 (`e1c4858`), CI green. Nothing is pending review, push, or
merge for any of the above. GitHub Issue #3 (Knowledge Ingestion) has
started: **Slice 3.1 (document data model + migration) is implemented,
tested, committed, and pushed, with a PR open and awaiting review** — see
`HANDOFF.md` for its exact commit hash and PR number. The next work, in
order:

1. Get the Issue #3 Slice 3.1 PR reviewed and merged.
2. After that, with an explicit go-ahead: Slice 3.2 of GitHub Issue #3
   (the next piece of Knowledge Ingestion — not started; do not assume
   which piece without checking `HANDOFF.md`/the Issue #3 plan first).
