...
(Knowledge Ingestion) has started: **Slice 3.1 (document data model +
migration) is merged** — PR #17, squash commit `79d4787`. **Slice 3.2
(`StorageProvider` abstraction) is merged** — PR #18, squash commit
`941c1a7`. **`main`/`origin/main` are currently at `941c1a7`.** A
**pre-merge correctness/security review of Slice 3.2 found and fixed a
real gap** (raw filesystem errors/paths could escape the module) — the
fix landed *after* PR #18 was already merged, so it's on a **new,
separate branch/PR**, not yet opened as of this line — see "Component
status" and "Immediate priorities" below, and `HANDOFF.md` for exact
detail.
...
(4/4 checks), and **merged into `main` as squash commit `941c1a7`**.
**`main`/`origin/main` are currently at `941c1a7`.** A pre-merge
...
already been merged — the fix is on branch
`issue-3-slice-3-2-storage-error-handling-fix`, cut from `941c1a7` — see
`HANDOFF.md` for its exact commit hashes and PR number once opened. The
old
...
(Knowledge Ingestion) has started: Slice 3.1 (merged) adds the
`documents`/`document_chunks` database schema (migration `0004`); Slice
3.2 (PR open) adds the `StorageProvider` abstraction — no upload API,
text extraction, chunking, embedding, retrieval, generation, chat,
search, or voice exists yet. Redis distributed rate limiting's
...
| Document upload & ingestion pipeline | PARTIALLY IMPLEMENTED (schema + storage abstraction only) | **Slice 3.1 — MERGED** (`79d4787`, PR #17): `documents`/`document_chunks` tables (migration `0004`, `backend/app/models/document.py`/`document_chunk.py`) — a native `DocumentStatus` enum matching the documented lifecycle (`UPLOADED → PROCESSING → PARSED → CLEANED → CHUNKED → EMBEDDED → INDEXED → READY/FAILED`), workspace-scoped ownership and cascade delete, `UNIQUE(workspace_id, checksum_sha256)`/`UNIQUE(storage_key)`/`UNIQUE(document_id, chunk_index)` constraints. Deliberately no embedding column yet (see `docs/DATA_MODEL.md`). **Slice 3.2 — MERGED** (`941c1a7`, PR #18): `StorageProvider` abstraction (`backend/app/services/storage_provider.py`) — a `Protocol` plus `LocalStorage` (filesystem-backed), path-traversal-safe key resolution (including symlink escapes), settings-driven factory (`storage_provider`/`storage_local_root`), mirroring `EmailProvider`'s pattern exactly. **A pre-merge correctness/security review found a genuine gap** — `save()`/`delete()`/`exists()` had no filesystem-error handling, so a raw `OSError`/`PermissionError` (including the absolute configured storage root in its message) could escape the module — **but PR #18 was merged before the fix could land in it; the fix is on a new, separate branch/PR, not yet merged as of this row** — see `docs/SECURITY.md` "Upload & document safety" and `HANDOFF.md`. **No upload API, text extraction, chunking, background processing, or embedding code exists** — those are later slices; nothing yet calls `get_storage_provider()`. See `HANDOFF.md` for exact branch/commit/PR detail. |
...
merged — PR #18 (`941c1a7`), CI green. Nothing is pending review, push,
or merge for any of the above. **A pre-merge correctness/security review
of Slice 3.2 found and fixed a real gap (raw filesystem errors/paths
could escape `StorageProvider`) — its fix landed after PR #18 was
already merged, so it's implemented, tested, and committed on a new,
separate branch, not yet pushed/PR'd as of this line** — see
`HANDOFF.md` for exact status. The next work, in order:

1. Push the fix branch, open its PR, get it reviewed and merged.
2. After that, with an explicit go-ahead: Slice 3.3 of GitHub Issue #3
   (the document upload endpoint, which consumes `StorageProvider` — not
   started; do not assume further detail without checking `HANDOFF.md`/
   the Issue #3 GitHub issue first).
