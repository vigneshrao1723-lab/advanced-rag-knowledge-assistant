...
| Distributed rate limiting / abuse protection (Redis) | Design: **DESIGNED**. Slice 1: **IMPLEMENTED, COMMITTED, PUSHED, MERGED** (`46ef03b`, PR #11). Slice 2: **IMPLEMENTED, COMMITTED, PUSHED, MERGED** (`5391a78`, PR #12). Abuse layer Slice 3a (Redis primitives only): **IMPLEMENTED, UNCOMMITTED, REAL-REDIS VALIDATED** (working tree only; all 32 tests passed 3 consecutive times against a real Redis + real PostgreSQL, executed inside the `compose-backend-1` container via Docker service hostnames after the host shell's published-port path proved broken — not yet committed, and **not the same as a production validation or a clean host-side run of the current 215-test full suite** — see `HANDOFF.md` for exact method/evidence). Abuse layer Slice 3b/3c (rule table, `AbuseDecisionEngine`, endpoint wiring): **NOT STARTED**. | [ADR 0006](docs/DECISIONS/0006-redis-distributed-rate-limiting-abuse-protection.md) documents the full architecture; its "Implementation status" table tracks exact per-component Designed/Implemented/Tested/Committed/Production-validated state. **Slice 1 — merged into `main` as `46ef03b`** added the foundation: the `redis` Python dependency; Redis settings (`REDIS_URL`, socket timeouts, `TRUSTED_PROXY_CIDRS`) in `app/core/config.py`; a connection abstraction (`app/core/redis_client.py`); a centralized key-builder + HMAC-SHA256 email identifier (`app/core/redis_keys.py`); a trusted-proxy IP-resolution function (`app/core/ip_resolution.py`, ADR §9a); the multi-key atomic Lua token-bucket engine (`RedisTokenBucketLimiter`/`DimensionSpec` in `app/core/rate_limit.py`, ADR §8/§10 — one Lua `EVAL` per operation over all of that operation's dimension keys, all-or-nothing); and a `redis` service in `infra/compose/docker-compose.yml`/CI. **Slice 2 — merged into `main` as `5391a78`** wires all of it into every `enforce_*_rate_limit` dependency: `register`, `login`, `refresh`, `forgot-password`, `reset-password` now all attempt the Redis engine first (using `resolve_client_ip()` for IP, HMAC-hashed email for `login`/`forgot-password`'s account dimension, the refresh-token cookie's session ID for `refresh`), falling back to `FixedWindowRateLimiter` per ADR §13. **This is now active in the committed (`main`) codebase.** Also adds the `rate_limit_hash_key`/`RATE_LIMIT_HASH_KEY` setting the HMAC construction uses, structured logging on the Redis-failure fallback path (ADR §13's requirement, `_log_redis_fallback()`), and corrected the two stale "not wired into any endpoint yet" docstrings from Slice 1. `/api/v1/health/ready` still deliberately does not check Redis (ADR §13, unchanged). Tested: 170 tests committed with Slice 1, 13 additional HTTP-level tests committed with Slice 2 — **183 total, on `main`** — see `HANDOFF.md` for exact numbers and what remains (the deterministic abuse layer). **Slice 3a (uncommitted, working-tree-only) adds the abuse layer's low-level Redis primitives**: `app/core/abuse_keys.py` (the `abuse:` key namespace) and `app/core/abuse_state.py` (atomic Lua-scripted record/reset operations for R1–R5's counters, R3's distinct-IP HyperLogLog, the strict-throttle bucket, and the temporary-block flag — reusing the existing `RedisTokenBucketLimiter`/`DimensionSpec` machinery for the strict bucket, no new engine). Fully parameterized — no rule thresholds are hardcoded; those are Slice 3b's rule table. 32 new tests (`backend/tests/test_abuse_state.py`) **passed 3 consecutive times against a real Redis and real PostgreSQL**, run inside the `compose-backend-1` container over the Docker-internal `db`/`redis` hostnames (the WSL host shell's published-port path to those same containers was independently confirmed broken — TCP handshake succeeds, protocol read is reset — an environment fault, not a Slice 3a defect). Live key inspection during that validation confirmed `abuse:*` keys are created with the expected types (failcount/HLL strings, strict-throttle hashes, block-flag strings) and every observed key had a finite, positive TTL, with no raw email substring found in any key name or value. `ruff`/`mypy` clean (both on host and inside the container). This was **not** a run of the current 215-test full suite: the container's baked image was stale (missing 8 pre-existing `test_rate_limit_wiring.py` tests, collecting 207) and its `EMAIL_PROVIDER=smtp` app-runtime setting caused 5 unrelated `test_password_reset.py` failures that a diagnostic `EMAIL_PROVIDER=console` override (invocation-only, no file changed) resolved to 207/207 — neither figure should be read as "the current 215-test suite passed." No endpoint wiring exists — `register`/`refresh`/`login`/`forgot-password`/`reset-password` are all byte-for-byte unchanged from Slice 2. |
