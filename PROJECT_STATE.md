...
**Redis Slice 1 (foundation + token-bucket engine) is IMPLEMENTED,
COMMITTED, PUSHED, and MERGED into `main`** as squash commit `46ef03b`
via PR #11. **Slice 2 (wiring that engine into every
`enforce_*_rate_limit` dependency) is IMPLEMENTED but exists only as
uncommitted working-tree changes** on branch
`issue-redis-rate-limiting-slice-2`, cut from the merged `main` — see
"Immediate priorities" and `HANDOFF.md`.
(repository: `vigneshrao1723-lab/advanced-rag-knowledge-assistant`,
currently checked out on `issue-redis-rate-limiting-slice-2`)
...
and **merged into `main` as squash commit `46ef03b`**. **`main`/
`origin/main` are currently at `46ef03b`.** The old
`issue-redis-rate-limiting` branch still exists locally and on `origin`
(GitHub did not auto-delete it) but has no content not already in `main`;
work continues on a fresh branch, `issue-redis-rate-limiting-slice-2`, cut
from the merged `main`. The old feature branch (`issue-2-authentication-workspaces`) still
...
supporting infrastructure) is implemented, committed, and **merged into
`main`** (`46ef03b`, PR #11); Slice 2 (wiring that engine into every
`enforce_*_rate_limit` dependency) is implemented but exists only as
**uncommitted working-tree changes** on `issue-redis-rate-limiting-slice-2`
— see the dedicated component-status row below. The deterministic
...
| Backend application (`backend/`) | IMPLEMENTED | Config, structured logging, request-ID middleware, centralized error handling, SQLAlchemy + Alembic, health/readiness (Issue #1) — plus (Issue #2) auth/workspace/password-recovery/audit-logging services, repositories, schemas, API routes, HttpOnly-cookie + CSRF middleware, plus (Redis Slice 1, **merged into `main`** as `46ef03b`) the Redis-backed token-bucket engine and its supporting infrastructure — **not yet wired into any endpoint in the committed codebase**; the wiring itself (Slice 2) exists only as uncommitted working-tree changes on `issue-redis-rate-limiting-slice-2` — described in the "Distributed rate limiting" row below. Verified on `main` alone (Slice 1 only): `ruff check` clean, `mypy` clean (74 files), `pytest` **170/170 passing** (real Postgres + real Redis). The full working tree (Slice 1 + uncommitted Slice 2, including this checkpoint's review-fix pass) separately verifies at `ruff`/`mypy` clean, `pytest` **183/183 passing** — not yet part of any commit. |
...
| Distributed rate limiting / abuse protection (Redis) | Design: **DESIGNED**. Slice 1: **IMPLEMENTED, COMMITTED, PUSHED, MERGED** (`46ef03b`, PR #11). Slice 2: **IMPLEMENTED, UNCOMMITTED** (working tree only, branch `issue-redis-rate-limiting-slice-2`). Abuse layer: **NOT STARTED**. | [ADR 0006](docs/DECISIONS/0006-redis-distributed-rate-limiting-abuse-protection.md) documents the full architecture; its "Implementation status" table tracks exact per-component Designed/Implemented/Tested/Committed/Production-validated state. **Slice 1 — merged into `main` as `46ef03b`** added the foundation: the `redis` Python dependency; Redis settings (`REDIS_URL`, socket timeouts, `TRUSTED_PROXY_CIDRS`) in `app/core/config.py`; a connection abstraction (`app/core/redis_client.py`); a centralized key-builder + HMAC-SHA256 email identifier (`app/core/redis_keys.py`); a trusted-proxy IP-resolution function (`app/core/ip_resolution.py`, ADR §9a); the multi-key atomic Lua token-bucket engine (`RedisTokenBucketLimiter`/`DimensionSpec` in `app/core/rate_limit.py`, ADR §8/§10 — one Lua `EVAL` per operation over all of that operation's dimension keys, all-or-nothing); and a `redis` service in `infra/compose/docker-compose.yml`/CI. **None of this is wired into any endpoint in the committed (`main`) codebase** — every `enforce_*_rate_limit` dependency as actually merged still calls only `FixedWindowRateLimiter`, unchanged. **Slice 2 exists only as uncommitted working-tree changes** on `issue-redis-rate-limiting-slice-2` — it would wire the engine into every `enforce_*_rate_limit` dependency (`register`, `login`, `refresh`, `forgot-password`, `reset-password` attempting the Redis engine first, using `resolve_client_ip()` for IP, HMAC-hashed email for `login`/`forgot-password`'s account dimension, the refresh-token cookie's session ID for `refresh`, falling back to `FixedWindowRateLimiter` per ADR §13), and adds the `rate_limit_hash_key`/`RATE_LIMIT_HASH_KEY` setting the HMAC construction uses — but **none of this is active in the committed codebase** until Slice 2 is itself reviewed and committed. A dedicated read-only security/architecture review of Slice 2 (0 P0, 3 P1, 3 P2, 2 P3) led to fixes still within the uncommitted working tree: structured logging on the Redis-failure fallback path (ADR §13's requirement), corrected stale docstrings, and 8 new HTTP-level tests (Redis-key creation for `refresh`/`forgot-password`, an IP-only-dimension test for `reset-password`, Tier A fallback for all three, an endpoint-driven multi-dimension atomicity test, and a cross-user account-isolation test). `/api/v1/health/ready` still deliberately does not check Redis (ADR §13, unchanged). Tested: 51 tests committed with Slice 1 (170 total, on `main`); 13 additional HTTP-level tests exist only in the uncommitted Slice 2 working tree (183 total there) — see `HANDOFF.md` for exact numbers and what remains (Slice 2 review/commit, then the deterministic abuse layer). |
...
| Testing (unit/integration/E2E/security) | PARTIALLY IMPLEMENTED | Backend, **committed (`main`, `46ef03b`)**: **170 pytest tests** (119 from Issue #2 + 51 from Redis Slice 1). Backend, **full working tree** (Slice 1 + uncommitted Slice 2, post review-fix pass): **183 pytest tests** (170 + 13 from the uncommitted Slice 2 HTTP-level wiring tests) — not yet part of any commit. Coverage includes real-database integration tests, cross-workspace-isolation/IDOR tests, CSRF tests (missing/mismatched/valid token, cross-client, login-CSRF, safe-method exemption), explicit `Set-Cookie` attribute assertions (HttpOnly/Path/SameSite), CORS preflight tests, an explicit "`Authorization: Bearer` alone does not authenticate and does not satisfy CSRF" test, password-reset security tests (enumeration resistance, single-use/expiry, cross-user isolation, session invalidation), real-Redis tests for the token-bucket engine (config validation, connection/availability, key construction/HMAC identifier, trusted-proxy IP resolution, first-request/consumption/rejection/TTL/cost semantics, multi-key all-or-nothing atomicity sequential and concurrent, the Redis-unavailable failure boundary), and (uncommitted, Slice 2) HTTP-level wiring tests through the real endpoints — real-Redis key creation via `login`/`refresh`/`forgot-password`, an IP-only-dimension proof for `reset-password`, Tier A fallback and Tier B fail-open via dependency override for all five endpoints, the unconfigured-vs-unreachable distinction, spoofed-`X-Forwarded-For` ignored by default, an endpoint-driven multi-dimension atomicity regression test, and a cross-user account-isolation test. Frontend: **48 vitest tests** — forms, auth state, nav, workspace switching/permission-sensitive UI, password-recovery pages, and explicit regression tests proving no auth token ever reaches `localStorage`/`sessionStorage`/an `Authorization` header. **No E2E browser suite exists** — Playwright has not been introduced (see the dedicated row above); this project's own live-Docker verification (curl/Python against the running containers) is not a substitute for it and is not represented as one. |
...
- **Rate limiting in the committed (`main`) codebase is still exclusively
  the in-process `FixedWindowRateLimiter`** — every `enforce_*_rate_limit`
  dependency, as actually merged (`46ef03b`), is byte-for-byte unchanged
  from before the Redis work and calls only the in-process limiter, same
  as pre-Redis behavior. The Redis-backed `RedisTokenBucketLimiter` engine
  exists and is merged (Slice 1) but **is not called from any endpoint in
  the committed state.** Wiring it in — every `enforce_*_rate_limit`
  dependency attempting `RedisTokenBucketLimiter` first (ADR §8/§10) and
  falling back to `FixedWindowRateLimiter` per ADR §13 — is Slice 2,
  implemented but **uncommitted, working-tree-only** on
  `issue-redis-rate-limiting-slice-2`; see the "Distributed rate
  limiting" component-status row above for the exact split. This
  repository's own `infra/compose/docker-compose.yml` (local dev) and
  `.github/workflows/ci.yml` (backend test job) both set `REDIS_URL` by
  default (committed with Slice 1), so a real Redis is present in those
  environments — but nothing in the *committed* codebase actually
  queries it yet. No production deployment of this project exists at all
  (ADR 0006 §1). The deterministic abuse-detection layer (ADR §11) does
  not exist in either committed or uncommitted form. See `HANDOFF.md` for
...
green. Redis Slice 1 is also done — PR #11 merged into `main` at commit
`46ef03b`, CI green (`main`/`origin/main` currently at `46ef03b`).
Nothing is pending review, push, or merge for either. The next work, in
order:

1. **Review, then commit and push Slice 2** (wiring the Redis engine into
   every `enforce_*_rate_limit` dependency) on branch
   `issue-redis-rate-limiting-slice-2`, and open a PR into `main` —
   implemented, working-tree-only today; its own read-only
   security/architecture review found 0 P0/3 P1/3 P2/2 P3, and the P1s
   plus the important P2 test-coverage gaps have since been fixed within
   the same uncommitted working tree (see `HANDOFF.md` for the exact
   list). This is the immediate next action.
2. **After Slice 2 merges: implement the deterministic (non-ML) abuse
   protection layer** ([ADR 0006](docs/DECISIONS/0006-redis-distributed-rate-limiting-abuse-protection.md)
   §11/§12) — not started in either committed or uncommitted form. Do not
   start this before Slice 2 has landed.
3. Introduce Playwright browser E2E coverage for the authentication/
   password-recovery flows — not yet started, no config or dependency
   exists.
4. After the above, begin GitHub Issue #3 (Knowledge Ingestion) — not
   started.
