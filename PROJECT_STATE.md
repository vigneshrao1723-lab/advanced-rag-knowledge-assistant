...
migration) is merged** — PR #17, squash commit `79d4787`. **`main`/
`origin/main` are currently at `79d4787`.** **Slice 3.2 (`StorageProvider`
abstraction) is IMPLEMENTED, TESTED, COMMITTED, and PUSHED — PR #18,
awaiting review** — not yet merged. See "Component status" and
...
`79d4787`**. **`main`/`origin/main` are currently at `79d4787`.** GitHub
Issue #3 Slice 3.2 (`StorageProvider` abstraction) is now implemented on
branch `issue-3-slice-3-2-storage-provider`, cut from `79d4787`, opened
as **PR #18** — see
`HANDOFF.md` for its exact commit hashes. The old
...
| Document upload & ingestion pipeline | PARTIALLY IMPLEMENTED (schema + storage abstraction only) | **Slice 3.1 — MERGED** (`79d4787`, PR #17): `documents`/`document_chunks` tables (migration `0004`, `backend/app/models/document.py`/`document_chunk.py`) — a native `DocumentStatus` enum matching the documented lifecycle (`UPLOADED → PROCESSING → PARSED → CLEANED → CHUNKED → EMBEDDED → INDEXED → READY/FAILED`), workspace-scoped ownership and cascade delete, `UNIQUE(workspace_id, checksum_sha256)`/`UNIQUE(storage_key)`/`UNIQUE(document_id, chunk_index)` constraints. Deliberately no embedding column yet (see `docs/DATA_MODEL.md`). **Slice 3.2 — IMPLEMENTED, TESTED, PR #18, not yet merged**: `StorageProvider` abstraction (`backend/app/services/storage_provider.py`) — a `Protocol` plus `LocalStorage` (filesystem-backed), path-traversal-safe key resolution (including symlink escapes), settings-driven factory (`storage_provider`/`storage_local_root`), mirroring `EmailProvider`'s pattern exactly. Every operation (`save`/`read`/`delete`/`exists`) guards its own filesystem calls so a raw `OSError`/`PermissionError` — and the absolute configured storage root that would otherwise appear in its message — never escapes the module (a genuine gap found and fixed in a pre-merge correctness review; see `docs/SECURITY.md` "Upload & document safety"). **No upload API, text extraction, chunking, background processing, or embedding code exists** — those are later slices; nothing yet calls `get_storage_provider()`. See `HANDOFF.md` for exact branch/commit/PR detail. |
...
(4/4 checks). Nothing is pending review, push, or merge for any of the
above. **Slice 3.2 (`StorageProvider` abstraction) is implemented,
tested, committed, and pushed — PR #18, awaiting review** — see
`HANDOFF.md` for its exact commit hashes. The next work, in
order:

1. Get the Issue #3 Slice 3.2 PR reviewed and merged.
