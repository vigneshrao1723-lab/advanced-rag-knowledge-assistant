"""Add password_reset_tokens and audit_logs.

Revision ID: 0003
Revises: 0002
Create Date: 2026-09-13

Per this issue's security requirements (Issue #2, extended scope):
`password_reset_tokens` stores only a hash of the reset token (never the
raw value), with expiry and single-use tracking; `audit_logs` is the
foundation for security-relevant event recording (docs/SECURITY.md "Audit
logging").
"""
