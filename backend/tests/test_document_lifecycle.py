"""HTTP-level tests for the Slice 3.6 lifecycle extension: the existing

    POST /api/v1/workspaces/{workspace_id}/documents/{document_id}/process

endpoint now continues past PARSED through CLEANED to CHUNKED, persisting
`document_chunks` rows. Real Postgres/Redis/filesystem, no mocks, matching
this project's established convention (see `tests/test_document_processing.py`,
which this file complements rather than duplicates — that file still owns
extraction-stage-specific coverage; this one owns cleaning/chunking/
persistence/resume/concurrency).
"""
...
# --- full pipeline: UPLOADED -> ... -> CHUNKED, with persisted chunks -----


def test_pdf_reaches_chunked_with_persisted_chunks(
...
def test_txt_reaches_chunked_and_chunks_are_persisted_with_correct_metadata(
    client: TestClient, db_session: DbSession
) -> None:
    _register(client)
    workspace = _create_workspace(client)
...
# --- resume behavior: PARSED / CLEANED are not terminal --------------------


def test_resume_from_parsed_reaches_chunked(client: TestClient, db_session: DbSession) -> None:
    _register(client)
    workspace = _create_workspace(client)
    document = _upload(
        client, workspace["id"], filename="a.txt", content=_TXT_BYTES, content_type="text/plain"
...
def test_resume_from_cleaned_reaches_chunked(client: TestClient, db_session: DbSession) -> None:
    _register(client)
    workspace = _create_workspace(client)
    document = _upload(
        client, workspace["id"], filename="a.txt", content=_TXT_BYTES, content_type="text/plain"
...
def test_processing_an_already_chunked_document_is_rejected_with_409(
    client: TestClient,
) -> None:
    _register(client)
    workspace = _create_workspace(client)
...
def test_concurrent_duplicate_chunk_insert_recovers_gracefully(
    ...
    response = _process(client, workspace["id"], document["id"])
    assert response.status_code == 200, response.text
    assert response.json()["status"] in ("CHUNKED", "CLEANED")  # never a raw 500
